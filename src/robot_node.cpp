...
class RosAriaNode
{
	...
	public:
		...
		void spin();
		void publish();
		void control(const velocity_obstacle::Pose::ConstPtr &);
	...
	myPublishCB(this, &RosAriaNode::publish)
{
	...
	// !!! port !!!
	n.param( "port", serial_port, std::string("localhost") );
	...
	cmddis_sub = n.subscribe("cmd_dis", 1, &RosAriaNode::control, this);
...
int RosAriaNode::Setup()
{
	...
	args->add("-rh"); //pass robot's serial port to Aria
...
void RosAriaNode::publish()
{
	...
	position.twist.twist.linear.x = robot->getVel(); //Aria returns velocity in mm/s.
...
void RosAriaNode::control(const velocity_obstacle::Pose::ConstPtr& pose){
	robot->move(pose->x);
	cout<<"Move the robot by "<<pose->x<<endl;
}
